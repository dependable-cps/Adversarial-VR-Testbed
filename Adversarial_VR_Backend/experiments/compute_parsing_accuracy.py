...
def retrieve_t5_model_dir(model, dset, debug):
    ...
    if dset == "diabetes":
        ...
        if model == "t5-small":
            ...
            raise NameError(f"no t5 model for model type {model}")
    elif dset == "cybersickness":
        if model == "t5-small":
            location = f"cybersickness_t5-small_epoch_23_lr_0.0001_batchsize_32_optimizer_adamw_downsample_1.0"
    else:
...
def main():
    ...
    if dset == "diabetes":
        ...
        test_suite = "./experiments/parsing_accuracy/cybersickness_test_suite.txt"
        config = "./configs/cybersickness-config.gin"
    else:
